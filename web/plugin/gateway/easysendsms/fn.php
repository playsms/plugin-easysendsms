...
// $uid : sender User ID
// $smslog_id : sms ID
function easysendsms_hook_sendsms($smsc, $sms_sender, $sms_footer, $sms_to, $sms_msg, $uid = '', $gpid = 0, $smslog_id = 0, $sms_type = 'text', $unicode = 0) {
	global $plugin_config;
	
	_log("enter smsc:" . $smsc . " smslog_id:" . $smslog_id . " uid:" . $uid . " to:" . $sms_to, 3, "easysendsms_hook_sendsms");
	
	// override plugin gateway configuration by smsc configuration
	$plugin_config = gateway_apply_smsc_config($smsc, $plugin_config);
	
	$sms_sender = stripslashes($sms_sender);
	if ($plugin_config['easysendsms']['module_sender']) {
		$sms_sender = $plugin_config['easysendsms']['module_sender'];
	}
	...
	if ($sms_to && $sms_msg) {
		...
		if ($c_message_id) {
			_log("sent smslog_id:" . $smslog_id . " message_id:" . $c_message_id . " smsc:" . $smsc, 2, "easysendsms_hook_sendsms");
			$db_query = "
				INSERT INTO " . _DB_PREF_ . "_gatewayEasySendSMS_log (local_smslog_id, remote_smslog_id)
